...
void JniConstants::Initialize(JNIEnv* env) {
    ...
    // references. Field ids and Method ids can be initialized lazily (and
    // racily) since they are not references. Also note, the FileDescriptor
    // class has a static member that is a FileDescriptor instance. Getting a
    // FieldId or MethodId initializes the class and static members and this
    // recurses into FieldId initialization which means this would require a
    // recursive_mutex.
