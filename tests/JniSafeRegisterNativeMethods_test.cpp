...
TEST(JniSafeRegisterNativeMethods, StringParsing) {
  constexpr ConstexprString<7> xyz = "abcdef";
  (void)xyz;
  auto xyz2 = MakeConstexprString("abcdef");
  (void)xyz2;

#if __cplusplus >= 201703L
  auto string_ptr = new ConstexprString{"abcdef"};
  (void)string_ptr;
  auto string_direct = ConstexprString{"abcdef"};
  (void)string_direct;
#endif
  ...
  {
    constexpr ConstexprStringErased v_str = "V";
...
TEST(JniSafeRegisterNativeMethods, FunctionTraits) {
  using traits_for_int_ret = FunctionTypeTraits<FN_ARGS_PAIR(test_function_traits::int_returning_function)>;
...
constexpr size_t SumUpVector(const ConstexprVector<T, kMaxSize>& vec) {
  size_t s = 0;
...
constexpr JniDescriptorNode MakeNode(ConstexprStringErased str) {
  return JniDescriptorNode{str};
}
...
#define JNI_TYPE_TRAIT_MUST_BE_SAME_FN(type_name, type_desc, ...)              \
  ...
  do {                                                                         \
    constexpr auto str_type_name = ConstexprStringErased(#type_name);          \
