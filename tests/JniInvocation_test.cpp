...
TEST(JNIInvocation, Debuggable) {
    ...
    const char* result = JniInvocation::GetLibrary(NULL, buffer, is_debuggable, get_library_system_property);
    ...
    result = JniInvocation::GetLibrary(kTestNonNull, buffer, is_debuggable, get_library_system_property);
    EXPECT_FALSE(result == NULL);
    if (result != NULL) {
