...
/*
 * Gets the default library for JNI invocation. The default library is libart.so, but this value
 * may be overridden for debuggable builds using the persist.sys.dalvik.vm.lib.2 system property.
 *
 * The |library| argument is the default library to use on debuggable builds (when
 * ro.debuggable=1). This argument may be NULL.
 *
 * The |buffer| argument is used for reading system properties and should be provisioned for
 * PROP_VALUE_MAX bytes.
 *
 * Returns pointer to the default invocation library.
 */
 ...
 * are initially null.
 * Returns new array on success, nullptr on failure. Raises an OutOfMemoryError exception if
 * allocation fails.
 */
