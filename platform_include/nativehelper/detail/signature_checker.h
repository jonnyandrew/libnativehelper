...
std::ostream& operator<<(std::ostream& os, const ConstexprStringView& str) {
  for (char c : str) {
    os << c;
  }
  return os;
...
MaybeMakeReifiedJniSignature() {
  if (!IsValidJniFunctionType<native_kind, T, fn>()) {
    PARSE_FAILURE(
        "The function signature has one or more types incompatible with JNI.");
  }
...
struct InferJniDescriptor {
  ...
  static constexpr ConstexprOptional<JniSignatureDescriptor<kMaxSize>> FromFunctionType() {
    ...
    if (reified_signature.ret.type_finality != kFinal) {
      ...
      PARSE_FAILURE(
          "Bad return type. Only unambigous (final) types can be used to infer a signature.");
    }
    ...
    for (size_t i = 0; i < reified_signature.args.size(); ++i) {
      ...
      if (arg_trait.type_finality != kFinal) {
        PARSE_FAILURE(
            "Bad parameter type. Only unambigous (final) types can be used to infer a signature.");
      }
