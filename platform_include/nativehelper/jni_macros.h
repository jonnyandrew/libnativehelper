 ...
 *   MAKE_JNI_[FAST_|CRITICAL_]NATIVE_METHOD - Create a checked JNINativeMethod{name, signature, func}.
 *   MAKE_JNI_[FAST_|CRITICAL_]NATIVE_METHOD_AUTOSIG - Same as above, but automatically infer signature.
 *
 * For convenience the following macros are defined:
 *   [FAST_|CRITICAL_]NATIVE_METHOD - Return JNINativeMethod given a class, function name, and signature.
 *   OVERLOADED_[FAST_|CRITICAL_]NATIVE_METHOD - Same as above but allows a separate function name identifier.
 *   [FAST_|CRITICAL_]NATIVE_METHOD_AUTOSIG - Return JNINativeMethod, signature is inferred from function type.
 *
 * The FAST_ prefix corresponds to functions annotated with @FastNative,
 * and the CRITICAL_ prefix corresponds to function annotated with @CriticalNative.
...
#define _NATIVEHELPER_JNI_MAKE_METHOD(kind, name, sig, fn)         \
  ({                                                               \
    JNINativeMethod tmp_native_method = {                          \
        (name), (sig), _NATIVEHELPER_JNI_MACRO_CAST(void * )(fn)   \
    };                                                             \
    tmp_native_method;                                             \
  })
