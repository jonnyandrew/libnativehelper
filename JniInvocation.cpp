...
#define ERROR_FORMAT_TYPE %lu
#define ERROR_TYPE DWORD
#else
#define FUNC_POINTER void*
#define ERROR_FORMAT_TYPE %s
#define ERROR_TYPE char*
#endif
...
bool JniInvocationImpl::Init(const char* library) {
  ...
  if (handle_ == NULL) {
    if (strcmp(library, kLibraryFallback) == 0) {
      // Nothing else to try.
      ALOGE("Failed to dlopen %s: " STRINGIFY(ERROR_FORMAT_TYPE), library, GetError());
      return false;
    }
    ...
    ALOGW("Falling back from %s to %s after dlopen error: " STRINGIFY(ERROR_FORMAT_TYPE),
          library, kLibraryFallback, GetError());
    library = kLibraryFallback;
    handle_ = OpenLibrary(library);
    if (handle_ == NULL) {
      ALOGE("Failed to dlopen %s: " STRINGIFY(ERROR_FORMAT_TYPE), library, GetError());
...
bool JniInvocationImpl::FindSymbol(FUNC_POINTER* pointer, const char* symbol) {
  *pointer = GetSymbol(handle_, symbol);
  if (*pointer == NULL) {
    ALOGE("Failed to find symbol %s: " STRINGIFY(ERROR_FORMAT_TYPE) "\n", symbol, GetError());
    CloseLibrary(handle_);
    handle_ = NULL;
    return false;
  }
