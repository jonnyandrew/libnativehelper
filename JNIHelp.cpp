...
class Constants final {
  ...
  private:
    ...
    static jmethodID FindMethodOrDie(JNIEnv* env,
                                     jclass klass,
                                     const char* name,
...
static bool getExceptionSummary(C_JNIEnv* env, jthrowable exception, std::string& result) {
    ...
    ScopedLocalRef<jclass> classClass(e, e->GetObjectClass(exceptionClass.get())); // can't fail
    jmethodID classGetNameMethod =
            e->GetMethodID(classClass.get(), "getName", "()Ljava/lang/String;");
...
static bool getStackTrace(C_JNIEnv* env, jthrowable exception, std::string& result) {
    ...
    ScopedLocalRef<jobject> stringWriter(e, e->NewObject(stringWriterClass.get(),
                                                         stringWriterCtor));
    if (stringWriter.get() == NULL) {
        return false;
    }

    ScopedLocalRef<jobject> printWriter(e, e->NewObject(printWriterClass.get(),
                                                        printWriterCtor,
                                                        stringWriter.get()));
