    ...
    const JNINativeMethod* gMethods, int numMethods)
{
    ...
    if (c.get() == NULL) {
        std::string tmp = android::base::StringPrintf("Native registration unable to find class '%s'; aborting...",
                                                      className);
        const char* msg;
        if (tmp.size() == 0) {
    ...
    if ((*env)->RegisterNatives(e, c.get(), gMethods, numMethods) < 0) {
        std::string tmp = android::base::StringPrintf("RegisterNatives failed for '%s'; aborting...", className);
        const char* msg;
        if (tmp.size() == 0) {
            // Allocation failed, print default warning.
